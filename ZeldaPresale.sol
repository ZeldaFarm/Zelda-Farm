...
contract ZeldaPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	function buy(uint256 _amount, address beneficiary) public nonReentrant {
        ...
        uint256 minUsdc = 5 * (10 ** usdcDecimals);

        require(_amount >= minUsdc,"Minimum 5 USDC required !!!");

		uint256 tokens = _amount.div(10 ** usdcDecimals).
						mul(TokenPerUSDC).div(10).
						mul(10 ** coinDecimals);
	...
	function getZTokensLeft() external view returns (uint256) {
	...
	function claimTokens(address _claimer) external nonReentrant {
		...
		require(tokensOwned[_claimer] > 0, "User should own some Z tokens");
		require(tokensUnclaimed[_claimer] > 0, "User should have unclaimed Z tokens");
		require(COIN.balanceOf(address(this)) >= tokensUnclaimed[_claimer], "There are not enough Z tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[_claimer];
		tokensUnclaimed[_claimer] = 0;
		COIN.safeTransfer(_claimer, callerTokensUnclaimed);
	...
	function withdrawUnsoldZ() external onlyOwner {
		uint256 amount = COIN.balanceOf(address(this)) - totalTokensSold;
		COIN.safeTransfer(msg.sender, amount);
	}

	function withdrawAllZ() external onlyOwner {
		COIN.safeTransfer(msg.sender, COIN.balanceOf(address(this)));
	}

	function changePrice(uint256 _newPrice) external onlyOwner {
		require(_newPrice >= TokenPerUSDC,"Price cannot be less than this !!!");
		require(_newPrice <= TokenPerUSDC.mul(4),"Price cannot be high than this !!!");
		TokenPerUSDC = _newPrice;
	}
}
